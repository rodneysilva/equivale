...
const ChatBell: Component = () => {
  ...
  const refresh = async () => {
    ...
    try { setUnread(await chatService.getUnreadCount()); }
  ...
  return (
    ...
    >
      <MessageCircle size={22} class="eq-brand hidden sm:block" />
      <MessageCircle size={18} class="eq-brand sm:hidden" />
      <Show when={unread() > 0}>
        <span
