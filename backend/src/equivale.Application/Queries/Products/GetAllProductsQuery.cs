...
public record GetAllProductsQuery(PaginationParams Pagination, string? SearchTerm = null, string? Category = null, List<string>? Tags = null, string? SellerId = null, string? CommunityId = null, string? SortBy = "recent") : IRequest<PagedResult<ProductDto>>;
...
public class GetAllProductsQueryHandler : IRequestHandler<GetAllProductsQuery, PagedResult<ProductDto>>
{
    ...
    public async Task<PagedResult<ProductDto>> Handle(GetAllProductsQuery request, CancellationToken cancellationToken)
    {
        var (items, total) = await _productRepository.GetPagedFilteredAsync(
            request.Pagination.Page, request.Pagination.PageSize, request.Category, request.SearchTerm, request.Tags, request.SellerId, request.CommunityId, request.SortBy, cancellationToken);
