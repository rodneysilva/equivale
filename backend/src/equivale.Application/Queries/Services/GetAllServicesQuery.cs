...
public record GetAllServicesQuery(PaginationParams Pagination, string? SearchTerm = null, string? Category = null, List<string>? Tags = null, string? ProviderId = null, string? CommunityId = null, string? SortBy = "recent") : IRequest<PagedResult<ServiceDto>>;
...
public class GetAllServicesQueryHandler : IRequestHandler<GetAllServicesQuery, PagedResult<ServiceDto>>
{
    ...
    public async Task<PagedResult<ServiceDto>> Handle(GetAllServicesQuery request, CancellationToken cancellationToken)
    {
        var (items, total) = await _serviceRepository.GetPagedFilteredAsync(
            request.Pagination.Page, request.Pagination.PageSize, request.Category, request.SearchTerm, request.Tags, request.ProviderId, request.CommunityId, request.SortBy, cancellationToken);
